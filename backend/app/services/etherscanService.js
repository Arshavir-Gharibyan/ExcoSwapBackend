...
const getAddressTransferEvents = async (address)=>{
    try {
        return  await axios.get(process.env.ETHERSCAN_API_URL +`?module=account&action=tokentx&address=${address}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${process.env.ETHERSCAN_API_KEY}`,
        );
    } catch (error) {
        return(error)
    }
...
const getTokenBalanceFromContractAddress = async (contractaddresses, address) =>{
    ...
    await Promise.all(contractaddresses.map(async (curr) => {
        const balance = await axios.get(process.env.ETHERSCAN_API_URL + `?module=account&action=tokenbalance&contractaddress=${curr}&address=${address}&tag=latest&apikey=${process.env.ETHERSCAN_API_KEY}`)
        result.push({
